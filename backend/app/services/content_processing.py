...
_SYSTEM_PROMPT = (
    ...
    "按以下业务标准判断（典型资料仅作参考，须结合实际正文敏感内容）："
    "L1 公开级：可公开传播、对外分享风险低，例如公开文章、政策、年报、课件、公司介绍。"
    "L2 内部参考级：内部可共享、不宜公开扩散，例如学习笔记、模板、行业简析、职能方法论。"
    "L3 受限级：需控制传播范围，例如核心方法论、问题清单、项目复盘。"
    "L4 商业秘密级：涉及客户敏感信息，例如客户方案、辅导周报、高管评价、访谈纪要。"
    "L5 严格商业秘密级：泄露将造成重大风险，例如客户经营数据、融资计划、合同、上市底稿。"
    "同一资料有多类内容时，按有正文证据的最高风险判断；公开年报中的已公开财务信息"
    "不因含金额自动判为 L5，空白合同模板不等同于已签合同。"
    "不能仅因没有显式保密标签就放弃判断；正文足够时依据上述标准给出建议和置信度。"
