...
async def build_response(
    ...
    *,
    ...
    return UploadSessionResponse(
        ...
        unuploaded_files=sum(item.ingest_task_id is None for item in visible_items),
        cancelled_files=sum(item.status == "cancelled" for item in value.items),
        unaccounted_files=max(0, value.total_files - len(value.items)),
