...
async def seed_dev_identities(session: AsyncSession) -> None:
    ...
    consultant_a.company_roles.append(
        UserCompanyRole(company_role="consultant", status="active")
    )
    ...
    manager_b.company_roles.append(
        UserCompanyRole(company_role="consultant", status="active")
    )
    ...
    director_d = User(
        id=USER_DIRECTOR, name="总监D", email="director.d@dev.local", status="active"
    )
    ...
    admin_e = User(
        id=USER_ADMIN_ONLY, name="管理员E", email="admin.e@dev.local", status="active"
    )
    admin_e.company_roles.append(UserCompanyRole(company_role="admin", status="active"))
    admin_e.company_roles.append(
        UserCompanyRole(company_role="consultant", status="inactive")
    )
...
def _asset(
    asset_id, *, title, scope, level, ai="A1", status="active", owner=USER_CONSULTANT,
    project_id=None, maintainer=USER_CONSULTANT, phase=None, zone="asset",
):
...
async def seed_dev_knowledge(session: AsyncSession) -> None:
    ...
    def _version(asset_id):
        return KnowledgeAssetVersion(
            id=uuid.uuid4(), asset_id=asset_id, version_no="v1",
            version_status="active", created_by=USER_CONSULTANT,
        )
    ...
    specs = [
        ...
        (_asset(KA_COMPANY_L4, title="医药集采渠道影响分析", scope="company", level="L4", ai="A4"),
        ...
        (_asset(KA_PROJECT_ALPHA, title="Alpha 项目供应链优化交付报告", scope="project",
                level="L2", project_id=PROJECT_ALPHA, phase="交付"),
         {"one_liner": "Alpha 项目供应链端到端优化交付报告",
          "detailed": "采购、仓储、物流三模块的诊断与优化建议。",
          "key_points": "采购优化\n仓储优化\n物流优化"},
         ["供应链", "流程优化"]),
        # project Beta L3：consultant_a 非 Beta 成员 → 发现 + 脱敏摘要，原文需申请。
        (_asset(KA_PROJECT_BETA_L3, title="Beta 项目客户访谈洞察", scope="project",
                level="L3", project_id=PROJECT_BETA, phase="诊断"),
         {"redacted_summary": "（脱敏）Beta 项目客户访谈关键洞察安全摘要。"},
         ["访谈", "洞察"]),
        # personal：仅 owner（consultant）本人可见。
        (_asset(KA_PERSONAL, title="个人方法论草稿", scope="personal", level="L2",
        ...
        (_asset(KA_COMPANY_ARCHIVED, title="已归档的旧组织诊断指南", scope="company",
                level="L1", status="archived"),
         {"one_liner": "已归档资产，默认不参与检索"},
         ["组织诊断"]),
        # 项目 material 资产（审核流测试用，无 seed review）。
        (_asset(KA_PROJECT_ALPHA_MATERIAL, title="Alpha 项目资料区待确认材料", scope="project",
                level="L2", project_id=PROJECT_ALPHA, phase="诊断", zone="material"),
         {"one_liner": "Alpha 项目资料区材料，待资产化确认", "detailed": "项目过程材料。"},
         ["资料区", "待确认"]),
        # 项目 material 资产（带 seed review，用于 /review 页面展示）。
        (_asset(KA_PROJECT_ALPHA_REVIEWABLE, title="Alpha 项目可复用方法论（待审）", scope="project",
                level="L2", project_id=PROJECT_ALPHA, phase="行动辅导", zone="material"),
         {"one_liner": "Alpha 项目内已分享、待项目经理确认为资产", "detailed": "项目内方法论。"},
         ["方法论", "待审"]),
        # 项目 A4 资产：Agent 渠道请求 original 必须降级（不进原文上下文），human 不受限。
        (_asset(KA_PROJECT_ALPHA_A4, title="Alpha 项目 A4 受限交付物", scope="project",
                level="L2", ai="A4", project_id=PROJECT_ALPHA, phase="交付"),
         {"one_liner": "Alpha 项目 A4 资产：Agent 仅可用摘要层", "detailed": "A4 受限内容。"},
         ["A4", "受限"]),
        # 项目 L5 资产：普通 consultant 不可发现，不进 Agent 引用/可见明细。
        (_asset(KA_PROJECT_ALPHA_L5, title="Alpha 项目 L5 绝密材料", scope="project",
                level="L5", project_id=PROJECT_ALPHA, phase="诊断"),
         {"redacted_summary": "（脱敏）Alpha 项目 L5 安全摘要。"},
         ["L5", "绝密"]),
        # 项目 archived 资产：不进入 Agent 候选（asset_status=active 过滤）。
        (_asset(KA_PROJECT_ALPHA_ARCHIVED, title="Alpha 项目已归档旧材料", scope="project",
                level="L2", project_id=PROJECT_ALPHA, phase="售前", status="archived"),
    ...
    session.add_all([
        WeknoraKbMapping(
            scope="company", owner_user_id=None, project_id=None,
            weknora_kb_id="wk-kb-company", embedding_model_id="seed-embed",
            kb_name="company_kb", status="active",
        ),
        WeknoraKbMapping(
            scope="project", owner_user_id=None, project_id=PROJECT_ALPHA,
            weknora_kb_id=f"wk-kb-proj-{PROJECT_ALPHA}", embedding_model_id="seed-embed",
            kb_name="project_alpha_kb", status="active",
        ),
        WeknoraKbMapping(
            scope="project", owner_user_id=None, project_id=PROJECT_BETA,
            weknora_kb_id=f"wk-kb-proj-{PROJECT_BETA}", embedding_model_id="seed-embed",
            kb_name="project_beta_kb", status="active",
        ),
    ])
