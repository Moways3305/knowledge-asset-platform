...
@pytest.mark.parametrize("count", [1301, 5000])
async def test_transport_manifest_accepts_large_selections(client, count):
    ...
    response = await client.post(
        ...
        json={
            ...
            "total_transport_batches": count,
            "manifest": manifest,
        },
    )
    assert response.status_code == 200
    body = response.json()
    assert body["total_files"] == count
    assert body["total_batches"] == count
