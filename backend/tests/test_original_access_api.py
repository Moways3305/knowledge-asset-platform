...
async def _mk_asset(db_session, *, scope="project", level="L3", project_id=PROJECT_BETA, status="active") -> uuid.UUID:
    """建一个跨项目/公司资产用于原文申请测试（USER_CONSULTANT 非 BETA active 成员）。"""
    aid = uuid.uuid4()
    db_session.add(
        KnowledgeAsset(
            id=aid, title="跨项目 L3 资产", scope=scope, zone="asset",
            asset_type="case", owner_user_id=USER_PROJECT_MANAGER, project_id=project_id,
            confidentiality_level=level, asset_status=status,
        )
    )
...
async def test_undiscoverable_asset_404(client):
    # 不存在的资产 → 404，不泄露存在。
    r = await client.post(_req_path(uuid.uuid4()), headers=_hdr(USER_CONSULTANT), json={"reason": "x"})
...
async def test_repeat_request_reuses_pending(client, db_session):
    ...
    count = (
        await db_session.execute(
            select(func.count()).select_from(OriginalAccessRequest).where(OriginalAccessRequest.asset_id == aid)
        )
    ).scalar_one()
    assert count == 1
...
async def test_pm_can_approve_creates_grant(client, db_session):
    ...
    r = await client.post(f"/api/v1/original-access/requests/{rid}/approve", headers=_hdr(USER_BOSS), json={"note": "ok"})
    assert r.status_code == 200, r.text
    assert r.json()["status"] == "approved" and r.json()["grant"]["status"] == "active"
    assert r.json()["grant"]["expires_at"] is not None
    # grant 落库且 active。
    g = (await db_session.execute(select(AccessGrant).where(AccessGrant.asset_id == aid))).scalar_one()
    assert g.grantee_user_id == USER_CONSULTANT and g.status == "active"


async def test_director_can_approve(client, db_session):
    aid, rid = await _create_pending(client, db_session)
    r = await client.post(f"/api/v1/original-access/requests/{rid}/approve", headers=_hdr(USER_DIRECTOR), json={})
    assert r.status_code == 200 and r.json()["status"] == "approved"


async def test_admin_cannot_approve(client, db_session):
    aid, rid = await _create_pending(client, db_session)
    r = await client.post(f"/api/v1/original-access/requests/{rid}/approve", headers=_hdr(USER_ADMIN_ONLY), json={})
...
async def test_consultant_cannot_approve(client, db_session):
    ...
    r = await client.post(f"/api/v1/original-access/requests/{rid}/approve", headers=_hdr(USER_CONSULTANT), json={})
    assert r.status_code == 403
...
async def test_grant_enables_then_revoke_disables_original(client, db_session):
    ...
    # 审批 → grant
    appr = await client.post(f"/api/v1/original-access/requests/{rid}/approve", headers=_hdr(USER_BOSS), json={})
    ...
    rv = await client.post(f"/api/v1/original-access/grants/{gid}/revoke", headers=_hdr(USER_BOSS), json={"reason": "no longer needed"})
...
async def test_expired_grant_not_live(client, db_session):
    ...
    # 直接建一条已过期 grant。
    g = AccessGrant(
        asset_id=aid, grantee_user_id=USER_CONSULTANT, grant_type="original_access",
        granted_by_user_id=USER_BOSS, status="active",
...
async def test_audit_actions_and_no_leak(client, db_session):
    ...
    assert {"access.original_requested", "access.original_approved", "access.original_grant_revoked"} <= actions
    rows = (
        await db_session.execute(
