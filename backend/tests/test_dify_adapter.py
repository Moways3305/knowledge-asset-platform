...
async def _insert_rule(
    ...
    *,
    ...
    rule = AgentWhitelistRule(
        provider="dify",
        agent_identifier=f"dify-app-{uuid.uuid4().hex[:8]}",
        agent_name="测试 Dify 接入",
        ...
        enabled=enabled,
        external_app_id="dify-secret-app-id",
        external_workflow_id="dify-secret-wf-id",
    )
    db_session.add(rule)
...
async def test_external_missing_bearer_rejected(client, db_session):
    ...
    resp = await client.post(
        RETRIEVAL,
        json={
            "knowledge_id": f"project:{PROJECT_ALPHA}",
            "query": "q",
...
async def test_external_non_member_project_no_context(client, db_session):
    ...
    resp = await client.post(
        ...
        headers=_bearer(),
        json={
            "knowledge_id": f"project:{PROJECT_BETA}",
            "query": "客户",
            "metadata_condition": {"caller_user_id": str(USER_CONSULTANT)},
        },
    )
...
async def test_external_project_pinned_token_blocks_other_scopes(client, db_session):
    ...
    r_beta = await client.post(
        RETRIEVAL,
        headers=_bearer(),
        json={"knowledge_id": f"project:{PROJECT_BETA}", "query": "q", "metadata_condition": mc},
    )
    assert r_beta.json()["records"] == []
    # company → 空。
    r_company = await client.post(
        RETRIEVAL,
        headers=_bearer(),
        json={"knowledge_id": "company", "query": "q", "metadata_condition": mc},
    )
    ...
    r_all = await client.post(
        RETRIEVAL,
        headers=_bearer(),
        json={"knowledge_id": "all", "query": "q", "metadata_condition": mc},
    )
    assert r_all.json()["records"] == []
    # 锁定项目本身 → 有记录。
    r_alpha = await client.post(
        RETRIEVAL,
        headers=_bearer(),
        json={"knowledge_id": f"project:{PROJECT_ALPHA}", "query": "q", "metadata_condition": mc},
    )
...
async def test_tool_company_only_token_rejects_project_and_all(client, db_session):
    ...
    r_proj = await client.post(
        TOOL,
        headers=_bearer(),
        json={"caller_user_id": str(USER_CONSULTANT), "query": "q", "scope": "project"},
    )
    ...
    r_all = await client.post(
        TOOL, headers=_bearer(), json={"caller_user_id": str(USER_CONSULTANT), "query": "q"}
    )
    ...
    r_comp = await client.post(
        TOOL,
        headers=_bearer(),
        json={"caller_user_id": str(USER_CONSULTANT), "query": "q", "scope": "company"},
    )
...
async def test_tool_search_agent_channel(client, db_session):
    ...
    resp = await client.post(
        ...
        headers=_bearer(),
        json={
            "caller_user_id": str(USER_CONSULTANT),
            "query": "如何做供应链优化？",
            "scope": "project",
        },
    )
