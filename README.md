...
- **外部 Agent 接入**：provider 中立的外部 Agent / 工作流网关，当前主接入面为 **WorkBuddy MCP**（`integrations/workbuddy-mcp/`）；Dify 为 legacy 兼容适配器（保留可用，新接入不再用）。Agent 不拥有独立权限，完全跟随调用人（按 token 绑定的真实用户）权限，经权限网关裁剪与脱敏。
