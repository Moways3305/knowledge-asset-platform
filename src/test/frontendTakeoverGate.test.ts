...
const routes: RouteContract[] = [
  {
    route: "/company-governance",
    component: "CompanyGovernancePage",
    guard: "viewCompanyKnowledge",
    owners: ["pages/CompanyGovernancePage.tsx"],
    apiModules: ["governance", "http"],
  },
  {
    route: "/release-notes",
  ...
  const visitRoutes = (node: ts.Node) => {
    if (ts.isJsxSelfClosingElement(node) && node.tagName.getText(app) === "Route") {
      ...
      if (route && route !== "/*") {
        ...
        findOwnership(node);
        if (component) ownership.set(route, { component, guard });
      }
    }
