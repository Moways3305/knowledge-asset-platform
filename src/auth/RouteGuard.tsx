...
export default function RouteGuard({ cap, children }: { cap: Capability; children: ReactNode }) {
  ...
  if (status === "anonymous") {
    return (
      <div className="state-box">
        <div className="state-title">请先登录</div>
        <p className="state-desc">登录后才能访问该功能。可在右上角身份菜单登录。</p>
  ...
  if (status === "error") {
    return (
      <div className="state-box">
        <div className="state-title">身份加载失败</div>
        <p className="state-desc">暂时无法确认你的身份，请稍后刷新重试。</p>
        <Link className="btn-primary" to="/">
          返回今日工作台
        </Link>
      </div>
  ...
  return (
    <div className="state-box">
      <div className="state-title">当前账号无此入口</div>
      <p className="state-desc">你当前的身份没有访问该功能的权限。如需开通，请联系平台管理员。</p>
      <Link className="btn-primary" to="/">
        返回今日工作台
      </Link>
    </div>
