...
const INITIAL = { name: "", dir: "", dirLabel: "", scope: "project", projectId: "", ownerId: "", enabled: true };
...
export default function WecomScanConfigForm({
  ...
  const ownerCandidates = useMemo(() => {
    if (values.scope === "project") {
      return values.projectId ? ownerOptions.filter((o) => o.project_ids.includes(values.projectId)) : [];
    }
  ...
  const handleSave = async () => {
    setSaveError(null);
    if (!values.name.trim()) { setSaveError("请填写配置名称"); return; }
    if (!values.dir.trim()) { setSaveError("请填写微盘目录内部标识"); return; }
    if (values.scope === "project" && !values.projectId) { setSaveError("项目级配置必须选择目标项目"); return; }
    if (!values.ownerId) { setSaveError("请选择待确认任务的业务归属人"); return; }
    setSaveBusy(true);
    const base = {
    ...
    <section className="ws-section">
      <div className="ws-detail-panel">
        <div className="ws-detail-head">
          ...
          <button className="btn-small" onClick={onClose} disabled={saveBusy}>关闭</button>
        </div>
        <div className="ws-form-grid">
          <label className="ws-form-field">
            <span className="ws-form-label">配置名称</span>
            <input className="ws-form-input" value={values.name} onChange={(e) => set("name", e.target.value)} placeholder="如：Alpha 项目交付目录" maxLength={200} />
          </label>
          <div className="ws-form-field">
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <span className="ws-form-hint">
                {values.dir
                  ? (values.dirLabel ? `已选择：${values.dirLabel}` : "已设置服务端目录配置")
                  : "从微盘空间/目录中选择，无需手填内部标识"}
              </span>
            </div>
            {pickerOpen && (
              <WecomDirectoryPicker
                onSelect={(ref, label) => { setMany({ dir: ref, dirLabel: label }); setPickerOpen(false); }}
              />
            )}
            <details className="ws-form-advanced" style={{ marginTop: 8 }}>
              ...
              <input className="ws-form-input" style={{ marginTop: 6 }} value={values.dir}
          ...
          <label className="ws-form-field">
            ...
            <select className="ws-form-input" value={values.scope} onChange={(e) => { setMany({ scope: e.target.value, ownerId: "", ...(e.target.value !== "project" ? { projectId: "" } : {}) }); }}>
          ...
          {values.scope === "project" && (
            <label className="ws-form-field">
              ...
              {projectOptions.length > 0 ? (
                <select className="ws-form-input" value={values.projectId} onChange={(e) => setMany({ projectId: e.target.value, ownerId: "" })}>
          ...
          <label className="ws-form-field">
            ...
            {(values.scope !== "project" || values.projectId) && ownerCandidates.length > 0 ? (
              <select className="ws-form-input" value={values.ownerId} onChange={(e) => set("ownerId", e.target.value)}>
        ...
        <div className="ws-form-actions">
          ...
          <button className="btn-small" onClick={onClose} disabled={saveBusy}>取消</button>
        </div>
      </div>
    </section>
  );
