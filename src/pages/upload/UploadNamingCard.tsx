  ...
  return (
    <section className="upload-section">
      ...
      <p className="page-help-line">
        命名格式、保密级别（L1–L5）/ AI 调用级别（A1–A4）与命名异常处理见{" "}
        ...
        {naming ? (
          <>
            ...
            <div className="naming-parse-note">
              规范化标题由 AI 根据文件名 + 抽取正文 + 平台命名规范生成；标「AI
              推断」为模型/默认推断字段，标「待人工校正」为缺乏依据的字段，请在下方人工校正区核对后提交。AI
              调用级别由保密级别推导。
            </div>
          </>
        ) : (
          <div className="naming-parse-note">
            选择文件并启动资产化后，平台将异步抽取正文并由 AI 生成规范化命名解析结果（一级类 /
            二级类 / 主题 / 对象/客户 / 日期 / 版本 / 保密级别 / AI 调用级别）。字段待后端返回。
          </div>
        )}
      </div>
    </section>
