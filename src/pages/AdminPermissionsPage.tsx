...
const ruleTypeLabel: Record<string, string> = {
  numeric: "数字阈值", toggle: "开关规则", fixed_path: "固定路径",
};
const groupLabel: Record<string, string> = {
  personal_flow: "个人知识流转", project_upgrade: "项目知识升格",
  access_request: "访问申请", asset_lifecycle: "资产生命周期",
};
const groupOrder = ["personal_flow", "project_upgrade", "access_request", "asset_lifecycle"];
const scopeLabel: Record<string, string> = {
  project: "项目级", company: "公司级", personal: "个人级", "project:": "项目级",
};
...
export default function AdminPermissionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { /* 身份获取失败按只读处理，写动作仍由后端兜底 403 */ }
    ...
    <div className="policy">
      <div className="kb-masthead">
        <div>
          ...
          <p className="kb-lead">配置知识流转阈值、访问申请策略与外部 Agent 接入注册。Boss / 咨询总监可修改业务权限规则；admin 只读；修改写入审计日志。</p>
        </div>
        <div className="kb-metrics">
          <div className="kb-metric"><div className="kb-metric-value">{stats.total}</div><div className="kb-metric-label">可配置规则</div></div>
          <div className="kb-metric"><div className="kb-metric-value">{stats.enabledAgents}</div><div className="kb-metric-label">已启用 Agent</div></div>
          <div className="kb-metric"><div className="kb-metric-value is-warning">{stats.modified}</div><div className="kb-metric-label">已调整</div></div>
          <div className="kb-metric"><div className={`kb-metric-value ${stats.riskAgents > 0 ? "is-warning" : "is-muted"}`}>{stats.riskAgents}</div><div className="kb-metric-label">风险提示</div></div>
        </div>
      </div>

      {!loading && !error && (
        canEditRules ? (
          <div className="policy-role is-edit">当前身份可<strong>查看并修改</strong>业务权限规则（Boss / 咨询总监）。</div>
        ) : isAdmin ? (
          <div className="policy-role is-readonly">当前身份为 <strong>admin（系统身份）</strong>：可查看权限规则，但不能修改业务权限规则。修改请使用 Boss / 咨询总监身份。</div>
        ) : null
      )}
      ...
      {error ? (
        <div className="kb-state">
          ...
          <p className="kb-state-desc">
            {isForbidden ? "权限规则查看仅对 admin / Boss / 咨询总监开放，顾问无权访问。" : "切换为授权身份后重试。"}
          </p>
          <button className="btn-secondary" onClick={() => void load()}>重试</button>
        </div>
      ) : loading ? (
        <div className="kb-state"><div className="kb-state-title">加载中…</div></div>
      ) : (
        <>
          {displayedGroups.map((group, gi) => (
            <section key={group} className="policy-group">
              <div className="policy-group-head">
                ...
                <span className="policy-group-rule">{(groupedRules[group] ?? []).length} 条规则</span>
              </div>
                  ...
                  return (
                    <div key={rule.rule_id} className="policy-row">
                      <div>
                        ...
                        <span className="policy-type">{ruleTypeLabel[rule.rule_type] ?? rule.rule_type}</span>
                      ...
                      <div className="policy-value">
                        {editing && rule.rule_type === "numeric" ? (
                          <span className="policy-edit-inline">
                            <input type="number" className="policy-input" min={0} value={editValue}
                              onChange={(e) => setEditValue(Math.max(0, Number(e.target.value)))} />
                            <span>{rule.unit ?? ""}</span>
                          </span>
                        ) : (
                          <span className={`policy-value-now ${isChanged(rule) ? "is-changed" : ""}`}>{valueText(rule)}</span>
                        )}
                      ...
                      <div>
                        ...
                        {rule.updated_by_name && (
                          <div className="policy-meta">最近修改：{rule.updated_by_name} · {fmtTime(rule.updated_at)}</div>
                        )}
                      </div>
                      <div className="policy-actions">
                        {!rule.editable ? (
                          ...
                          <span className="policy-readonly">{isAdmin ? "admin 只读" : "无修改权"}</span>
                        ) : editing ? (
                          <>
                            <button className="btn-small btn-small-primary" onClick={() => void applyUpdate(rule.rule_id, { value_number: editValue })}>保存</button>
          ...
          <section className="policy-group">
            <div className="policy-group-head">
              <span className="policy-group-no">{String(displayedGroups.length + 1).padStart(2, "0")}</span>
              <span className="policy-group-title">外部 Agent 接入注册</span>
              <span className="policy-group-rule">provider 中立网关白名单</span>
            </div>
            {agentNote ? (
              <div className="policy-role is-readonly">{agentNote}本区块来自 provider 中立后端兼容接口，不在前端伪造数据。</div>
            ) : agents.length === 0 ? (
              <div className="kb-state"><div className="kb-state-title">暂无已注册的外部 Agent 接入</div></div>
            ) : (
              <div className="policy-ledger">
                {agents.map((agent) => (
                  <div key={agent.id} className={`policy-row ${!agent.enabled ? "is-disabled" : ""}`}>
                    <div>
                      <span className="policy-name">{agent.agent_name}</span>
                      <div className="policy-key">
                        {agent.provider} · {agent.capability} · {agent.allowed_scope ? (scopeLabel[agent.allowed_scope] ?? agent.allowed_scope) : "全范围"} · 最高 {agent.max_confidentiality_level}
                      </div>
                    </div>
                    <div className="policy-value">
                      <span className={`policy-status ${agent.enabled ? "on" : "off"}`}>{agent.enabled ? "已启用" : "已停用"}</span>
                    </div>
                    <div className="policy-desc">{agent.risk_note ? <span className="policy-risk">{agent.risk_note}</span> : "—"}</div>
                    <div className="policy-actions">
                      <button className={agent.enabled ? "btn-small" : "btn-small btn-small-primary"} onClick={() => void toggleAgent(agent)}>
                        {agent.enabled ? "停用" : "启用"}
                      </button>
                    </div>
                  </div>
