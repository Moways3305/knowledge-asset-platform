...
import {
  ClipboardList, FileCheck2, ShieldCheck, KeyRound, Clock, AlertTriangle, RefreshCw,
...
const roleLabel: Record<string, string> = {
  boss: "Boss", consulting_director: "咨询总监", consultant: "顾问", admin: "管理员",
};
const projectRoleLabel: Record<string, string> = {
  project_manager: "项目经理", consultant: "顾问", coach: "辅导老师",
};
...
type Todo = {
  key: string; label: string; desc: string; count: number; to: string;
  severity: "default" | "warning" | "danger"; icon: typeof FileCheck2;
};
...
export default function HomeDashboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPendingReviews(reviews ? reviews.filter((r) => r.status.startsWith("pending")).length : null);
  ...
  const todos: Todo[] = [];
  const push = (t: Todo) => { if (t.count > 0) todos.push(t); };
  if (pendingIngest != null) push({ key: "ingest", label: "待确认入库任务", desc: "微盘扫描产生、等待资产化确认", count: pendingIngest, to: "/upload", severity: "default", icon: FileCheck2 });
  if (pendingReviews != null) push({ key: "review", label: "待我审核", desc: "升级 / 资产化确认审核", count: pendingReviews, to: "/review", severity: "default", icon: ShieldCheck });
  if (originalInbox != null) push({ key: "oa-inbox", label: "待处理原文访问申请", desc: "需我审批的原文授权", count: originalInbox, to: "/original-access", severity: "warning", icon: KeyRound });
  if (originalMine != null) push({ key: "oa-mine", label: "我发起的原文申请", desc: "处理中的原文访问申请", count: originalMine, to: "/original-access", severity: "default", icon: Clock });
  push({ key: "idx", label: "索引失败待处理", desc: "知识底座索引失败，可重试", count: idxFailed, to: isAdmin || isGovernance ? "/admin/ingest" : "/knowledge", severity: "danger", icon: AlertTriangle });
  push({ key: "overdue", label: "原文申请已超时", desc: "超过自动通过时限仍待处理", count: overdueOriginal, to: "/original-access", severity: "warning", icon: Clock });
  push({ key: "update", label: "资产待更新", desc: "标记为需更新的知识资产", count: needsUpdate, to: "/knowledge", severity: "warning", icon: RefreshCw });

  // 今日状态指标（来自真实运营洞察，按角色后端已裁剪）。
  const stats = insights ? [
    { key: "failed", label: "索引失败", value: idxFailed, tone: idxFailed > 0 ? "is-danger" : "", to: isAdmin || isGovernance ? "/admin/ingest" : "/knowledge" },
    { key: "skipped", label: "未索引底座", value: idxSkipped, tone: "", to: "/knowledge" },
    { key: "pending-oa", label: "原文待处理", value: pendingOriginal, tone: pendingOriginal > 0 ? "is-warning" : "", to: "/original-access" },
    { key: "update", label: "待更新", value: needsUpdate, tone: needsUpdate > 0 ? "is-warning" : "", to: "/knowledge" },
    { key: "archive", label: "归档候选", value: archiveCandidates, tone: "", to: "/knowledge" },
    { key: "reuse", label: "复用升格候选", value: reuseCandidates, tone: reuseCandidates > 0 ? "is-success" : "", to: "/knowledge" },
  ] : [];
    ...
    <div className="home">
      <div className="home-masthead">
        <div>
          ...
          <h2 className="home-greeting">{greeting()}，<span className="accent">{me?.name ?? "同事"}</span></h2>
      ...
      <div className="home-grid">
        <div className="home-col">
          ...
          <section className="home-section">
            <div className="home-section-head">
              <span className="home-section-title"><ClipboardList size={16} /> 我的待办</span>
              <span className="home-section-eyebrow">Action items</span>
            </div>
            {loading ? (
              <div className="kb-state"><div className="kb-state-icon"><ClipboardList size={20} /></div><div className="kb-state-title">加载中…</div></div>
            ) : todos.length > 0 ? (
                ...
                {todos.map((t) => {
                  ...
                  return (
                    <Link key={t.key} to={t.to} className={`home-todo sev-${t.severity}`}>
                      <span className="home-todo-icon"><Icon size={18} /></span>
              ...
              <div className="kb-state">
                <div className="kb-state-icon"><ShieldCheck size={20} /></div>
                <div className="kb-state-title">今日暂无待办</div>
                <p className="kb-state-desc">没有需要你处理的入库确认、审核或原文申请。可从右侧快捷入口进入日常工作。</p>
              </div>
            )}
          </section>
          ...
          {insights && (
            <section className="home-section">
              <div className="home-section-head">
                <span className="home-section-title"><Gauge size={16} /> 今日状态</span>
                <span className="home-section-eyebrow">{insights.window_days} 天窗口{!insights.title_visible && " · 运营视图"}</span>
              </div>
          ...
          {insights && insights.recommendations.length > 0 && (
            <section className="home-section">
              <div className="home-section-head">
                <span className="home-section-title"><Lightbulb size={16} /> 运营建议</span>
              </div>
        ...
        <aside className="home-aside">
          <div className="home-panel">
            <h4 className="home-panel-title"><Rocket size={13} /> 快捷入口</h4>
            ...
            {(isAdmin || isGovernance) && (
              <>
                <h4 className="home-panel-title" style={{ marginTop: 16 }}><ShieldAlert size={13} /> 管理后台</h4>
                <div className="home-quick">
                  {adminQuick.map((q) => {
                    const Icon = q.icon;
                    return (
          ...
          <div className="home-panel">
            <h4 className="home-panel-title"><Building2 size={13} /> 我的项目范围</h4>
            {projects && projects.length > 0 ? (
              projects.slice(0, 6).map((p) => (
                <Link key={p.id} to={`/project/${p.id}/knowledge`} className="home-scope-item">
                  <span className="home-scope-name">{p.name}</span>
                  <span className="home-scope-role">{p.status === "active" ? "进行中" : p.status}</span>
                </Link>
              ))
              ...
              me.projects.map((p) => (
                <div key={p.projectId} className="home-scope-item">
                  ...
                  <span className="home-scope-role">{projectRoleLabel[p.projectRole] ?? p.projectRole}</span>
                </div>
              ))
            ) : (
              <p className="home-scope-empty">暂无项目身份。可发现的公司知识仍可在知识资产库浏览。</p>
            )}
          </div>

          {insights && insights.recent_items.length > 0 && (
            <div className="home-panel">
              <h4 className="home-panel-title"><Clock size={13} /> 最近运营动态</h4>
