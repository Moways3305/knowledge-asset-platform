...
const SECTIONS: HelpSection[] = [
  {
    ...
    title: "账号与身份",
    items: [
      { term: "登录方式", text: "支持密码登录与企微 OAuth（企微登录需后端配置 WECOM_*，未配置时安全降级）。密码登录已接入（所有环境按邮箱+密码校验）；开发环境可不填密码用邮箱便捷登录。MFA / 找回密码 / 账户锁定为后续增强。" },
      { term: "登录审计", text: "登录成功/失败均审计，记录 login_method（password / wecom_oauth / dev_local）与安全元数据；绝不记录明文密码 / hash / 会话 token。" },
  ...
  },
  {
    ...
    items: [
      ...
      { term: "运营洞察", text: "知识首页右侧运营洞察为真实后端安全聚合：索引失败 / 解析异常、最近索引运维作业、原文申请待处理 / 超时 / 自动通过、归档候选 / 升格推荐。按角色范围裁剪：纯系统管理身份只看运营聚合且业务标题隐藏；业务治理角色看治理范围；项目角色 / 普通业务用户看本人与所在项目范围。统计来自真实表，不展示原文 / 文件名 / 底座内部 id；无运营项时如实显示「暂无需要处理的运营项」。" },
  ...
  },
  {
    ...
    items: [
      ...
      { term: "入库前置脱敏", text: "抽取成功后先做确定性规则实体脱敏（邮箱 / 手机号 / 固话 / 身份证 / 银行卡 / 长账号 / 金额 / 联系人 / 客户字段），平台侧外部 LLM 内容建议仅使用脱敏后文本；不可抽取文本则无法做文本级前置脱敏。WeKnora 底座及其 LLM 是已确认的受信任底座处理方，按该信任边界仍可接触原文做索引；原始文件保留供授权预览 / 溯源。不展示脱敏前后正文对比。" },
  ...
  },
  {
    ...
    items: [
      ...
      { term: "原文访问授权", text: "申请 / 审批 / 拒绝 / 撤销已接入真实后端；审批通过生成 active 授权并在运行时放行原文层，过期 / 撤销立即失效。L1/L2 申请超过「访问申请自动通过时限」未审批由后台任务自动通过并生成授权；L3/L4/L5 机密资产不自动通过。" },
      { term: "个人到项目提交", text: "个人知识进入项目须由知识所有者本人主动提交；项目经理审核个人到项目的提交。" },
      { term: "边界", text: "项目 material → asset 审核闭环已可用；项目资产 → 公司知识的升格审核暂未开放。" },
    ],
  },
  {
    id: "admin",
    ...
    items: [
      ...
      { term: "知识底座索引运维", text: "运维查看索引失败 / 解析滞留计数与最近失败资产（安全运营态）。可对筛选出的失败 / 跳过 / 待索引资产发起批量重试索引，对已进底座但解析异常的资产发起「重新解析」（受控重传刷新底座解析）；批量动作进入后台作业，面板展示作业安全统计与最近作业列表。重新解析是底座解析运维，不改变任何权限放行，不让未授权用户读到原文；refresh-parse 仍是只读对账。运营面板绝不展示业务原文 / 文件名 / WeKnora 内部 id / 存储引用。" },
      ...
      { term: "权限规则", text: "permission_rules 配置中心（Boss / 咨询总监可改、admin 只读）。跨项目 / 公司 L1/L2 原文默认放行开关与访问申请自动通过时限已运行时生效：关闭开关即让对应范围 L1/L2 原文回到「需申请」；其余部分规则仍为治理配置视图，运行时接入边界见各规则说明。" },
